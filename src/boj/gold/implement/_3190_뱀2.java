...
//			String d = st.nextToken(); // 왼L 오D 로 90도 회전
//			dir.put(t, d);
			int d = st.nextToken().equals("D") ? 0 : 1;
			direction.add(new Pos(t, d));
		}
		...
		while (true) {
			...
			for (int i = 0; i < direction.size(); i++) {
				if (direction.get(i).x == playTime) {
					if (direction.get(i).y == 0) {
						idx++;
						idx = idx == 4 ? 0 : idx;
					} else {
						idx--;
...
//			if (dir.containsKey(playTime)) {
//				if (dir.get(playTime).equals("D")) { // x초 후 오른쪽으로 90도 전환
//					idx++; // 우 하 좌 상
//					if (idx == 4) {
//						idx = 0; // 다시 우로
//					}
//				}
//				if (dir.get(playTime).equals("L")) { // x초 후 왼쪽으로 90도 전환
//					idx--; // 상 좌 하 우
//					if (idx == -1) {
//						idx = 3; // 상부터 시작
//					}
